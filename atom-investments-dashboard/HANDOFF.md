...
- **Button States:** Hover (shadow lift + color change), Focus (blue ring), Disabled (opacity 50%)
- **Forms:** Proper border-radius (6px), focus glow (blue ring at 3px offset)
...
| File | Purpose | Status |
|------|---------|--------|
| `src/components/common/Button.jsx` | Global button component | ✅ Complete |
| `src/styles/tokens.css` | Design tokens (colors, spacing) | ✅ Complete |
| `src/index.css` | Global styles | ✅ Complete |
| `src/components/Overview.jsx` | Dashboard main view | ✅ Complete |
| `src/components/Projects.jsx` | Project list | 🟡 Partial (needs redesign) |
| `src/components/Roadmap.jsx` | Timeline/calendar view | 🟡 Partial (needs redesign) |
| `src/pages/LoginPage.jsx` | Authentication page | ✅ Complete |
| `src/pages/ProfilePage.jsx` | User profile page | 🟡 Partial (needs avatar fix) |
| `src/mockData.js` | Mock data for development | ✅ Complete |
...
- Check `ENHANCEMENT-STRATEGY.md` for old design approach (reference only)
- All components should follow Button.jsx and Overview.jsx patterns
- When in doubt, check shadcn/ui GitHub repo for best practices
- Test focus states and keyboard navigation on every change
